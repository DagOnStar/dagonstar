...
- an initial unit test suite for configuration parsing, workflow defaults,
  dependency discovery, cycle validation, serialization, and runtime-secret
  validation;
...
- packaging cleanup for included DAGonStar packages.

The main engineering priorities are now:

1. expand automated tests from the current core unit tests to broader workflow,
   checkpoint, staging, and optional-integration coverage;
2. add typed interfaces to the public workflow, task, configuration, and staging
   APIs;
3. harden shell-command construction and path quoting for commands generated by
   tasks and data staging;
4. separate optional integration dependencies into install extras such as
